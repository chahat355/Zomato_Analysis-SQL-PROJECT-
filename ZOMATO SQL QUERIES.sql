...
#2. Which food item is ordered the most?
select food.f_id,
food.f_name, count(*) as total_orders
from food join order_details on
food.f_id = order_details.f_id
group by food.f_id,
food.f_name order by total_orders desc;

#3.Which customer spent the maximum amount overall?
...
#4.Find the highest rated restaurant (based on avg rating).
...
#5. What is the monthly revenue of Zomato?
...
#6. Find the top 5 customers with the highest number of orders.
select 
...
#7  Find the restaurants that generated the highest total revenue.
select 
restaurants.r_name,
...
#8.Which delivery partner has the best average delivery rating?
...
#9.  Use  view  and  list all orders with order_id, user name, restaurant id, food items, and delivery partner name.
...
#10 Find min and max order value for all the customers.
select * from users;
select * from orders;
select users.user_id,users.name ,
min(orders.amount),
...
#11. Find restaurant with most number of menu items.
...
#12 Find the food that is being sold at most number of restaurants.
...
#13 Find the average delivery time for each delivery partner.
select delivery_partner.partner_name as partner_name,
round(avg(orders.delivery_rating))as partner_rating from 
delivery_partner join orders on 
delivery_partner.partner_id = orders.partner_id 
group by partner_name;

#14 Find revenue per month for a restaurant
 select restaurants.r_name,
...
#17 Find most costly restaurants.
...
#18. Find delivery partner compenstion using the formula(#deliveries*100+1000*avg_rating)
select delivery_partner.partner_name,
